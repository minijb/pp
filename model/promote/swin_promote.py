...
class liner_prom(nn.ModuleList):
    def __init__(self, in_size, out_size, feature_shape) :
        ...
        if feature_shape == 8:
            self.conv = nn.Conv2d(in_size, internal_size, kernel_size = 3, stride = 1 , padding = 1, bias = False)
        elif feature_shape == 16:
            self.conv = nn.Conv2d(in_size, internal_size, kernel_size = 2 , stride = 2 , bias =  False)
            feature_shape = 8
        else:
